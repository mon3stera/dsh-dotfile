...
The outline is mounted through `conversation.session.header.utilities` and uses a fixed right-side panel so it does not replace the built-in details panel. Its lower edge stays above the composer, and the panel can be collapsed from the header button.
...
- insert:
    - id: dsh-plugin-outline
      name: dsh-plugin-outline/client
```
