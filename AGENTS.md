...
lib/
  ...
  dreamer.js               Dreamer instruction/brief/actions, shared read-only tool executors, archival
  dreamer-session.js       Per-pass Dreamer child-session driver (spawn/descriptor/route/dispose)
  dream-agent.js           Agent-plane registrar mounting the Dreamer tools for the dream preset
...
- Dreamer (default `dreamerMode: "session"`) runs one maintenance pass as a real child session of the parent: `lib/dreamer-session.js` spawns it through the host `agents.create` with header meta `{parentSession, origin: "subagent", agentPreset: "dream"}`, appends the `subagent/descriptor` event the subagent catalog's identity projection folds into a visible row, sends the material brief as the first user message, commits the Dreamer model route with `sessionController.agents.selectForNextRequest` (session-local selection only, never the deployment default), reads the verdict and every `tool/call`/`tool/result` pair back from the child's native log, and disposes the child. Passes are independent — a later pass inherits no conversation from an earlier one; cross-pass state lives only in the context database. The child appears in the parent's subagent catalog, never in the top-level sidebar, and its log is the pass's full audit trail.
- The `dream` agent preset (`preset/dream/`, installed by the preset installer alongside `context-compact`) mounts only the `dsh-magic-context/dream-agent` registrar: it registers the read-only maintenance tools into the host `tools` registry (executors shared with the legacy loop through `createDreamerExecutors`, workspace/scope resolved from `exec.agent.session` at call time) plus the Dreamer instruction section. The registrar also captures the host `sessions` service, so `session_context` can read a live (resident) source session — typically the triggering parent — under the unchanged scope checks; non-resident sources degrade to the stored fact/compartment summary, the same live-only semantics as the legacy loop. It deliberately mounts NO compaction group, shell, general filesystem, or delegation — a dream pass cannot compact, so context overflow fails the pass loudly as the designed too-heavy signal. If the preset is missing, the engine falls back to the legacy in-process `ctx.llm.streamAux()` loop; `dreamerMode: "loop"` forces that loop always. The profile preset also excludes the `session-dream-` id prefix in `sessionFilter`, so dream children never feed the memory pipeline.
